...
console.log(`${adet} adet sayinin toplamı:${toplam}`); */
...
dizi1.forEach((item, index) => {
  yeniSayiDizisi[index] = Math.abs(item * 2);
});

yeniSayiDizisi.sort((a, b) => b - a);
console.log(yeniSayiDizisi);
...
const ortalamaUstü = () => {
  const ortUstü = notlar1.filter((t) => t > ortalama);
  return ortUstü;
};
console.log(ortalamaUstü());
